...
2. Offer the Phase 1 workaround: walk the user back through the session
   ...
   running `/playbook-log` for each. User confirms one at a time.
3. Do not batch-write entries. Do not auto-approve. Each entry is an
   explicit `/playbook-log` invocation with user sign-off.
...
- Do not fake the distill flow by chaining `/playbook-log` invocations

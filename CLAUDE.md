...
This is the source repository for **Playbook** — a project-scoped harness memory tool built on top of Anthropic's `memory_20250818` tool. Phase 1 scaffold is in place: skill + slash commands + two hooks under `.claude/`. No code has been tested end-to-end against the live memory tool yet. The working directory is not a git repo.

**Positioning:** "the harness is the dataset" — Playbook is the structured trajectory dataset that differentiates a project's harness. Not a memory/notes product. This framing is load-bearing for every design decision.

**Primary use case:** handoff artifact across context resets (per Anthropic's harness-design-long-running-apps argument that resets beat in-place compaction for long tasks). Entries must be self-contained enough that a cold-start agent becomes productive by reading the playbook only — not by replaying prior conversation history.

Contents:
- `phase-1-spec.md` — the active spec. Read this before proposing anything. Covers file layout under `/memories/playbook/`, entry schema (7 types: correction, constraint, pattern, decision, dead-end, operator, open-question), Phase 1 commands, and the two Phase 1 hooks.
...
- Final project name is TBD — "playbook" is the working name. `phase-1-spec.md` §9 lists alternatives.

## Key design choices already made (per spec)

- Directory-backed store under `/memories/playbook/`, not a DB.
...
- Two Phase 1 hooks only: `SessionStart` for cold-start orientation, `UserPromptSubmit` for operator context injection.
...
- **Phase 1 — Capture + retrieve (current scope).** Scaffold `/memories/playbook/`, typed entries, `operator.yaml`, two hooks (cold-start view + operator injection), skill commands: `init`, `log`, `state`, `index`, `recall`. `distill` stubbed as propose-only. First E2E surface: `/playbook init`.
- **Phase 2 — Curation.** `/playbook distill` sub-agent reviews session transcripts, proposes typed entries as a reviewable diff. `/playbook promote <id>` moves entries up the tiers (raw → structured → constraint); constraint promotion enforces the back-pressure-target gate. Helpful/contradicted counters start incrementing. Conflict resolution — when new entries contradict old ones, the contradiction becomes a first-class object, not a silent overwrite (ACE grow-and-refine pattern).
- **Phase 3 — Compile.** `/playbook compile` regenerates `compiled/harness-reminders.md` (hook-injected) and `compiled/narrative.md` (human view) from the typed store deterministically. Lean-by-default emission (see design choices above). Diffs reviewable before apply.
...
- `.claude/skills/playbook/SKILL.md` — instruction layer the agent loads contextually
- `.claude/commands/playbook-init.md` — bootstrap `/memories/playbook/` (first E2E surface)
- `.claude/commands/playbook-log.md` — force-write a single typed entry
- `.claude/commands/playbook-index.md` — regenerate `index.json` from `entries/`
- `.claude/commands/playbook-recall.md` — search by tag / type / substring
- `.claude/commands/playbook-state.md` — update `state.json` from working tree
- `.claude/commands/playbook-distill.md` — Phase 2 stub (do not implement in Phase 1)
- `.claude/hooks/session-start.sh` — cold-start orientation reminder
- `.claude/hooks/user-prompt-submit.sh` — operator context injection
- `.claude/settings.local.json` — registers the two hooks
...
First end-to-end test: fresh Claude Code session in this repo → `/playbook-init`. Expect the bootstrap flow to create `/memories/playbook/{HEAD.md, index.json, state.json, operator.yaml, entries/.keep, compiled/.keep, archive/.keep}`.
